...
export class OrdersService {
  private baseUrl = 'http://localhost:8080/api/payment/orders'; // Base URL for the payment API

  constructor(private http: HttpClient) {}

  // Method for merchants to request payment from customers
  createPaymentRequest(orderRequest: Order): Observable<any> {
    return this.http.post<any>(`http://localhost:8080/api/payment/orders/requestPayment`, orderRequest);
  }
  ...
  executePaymentToMerchant(orderRequest: Order, customerAccount: account, orderId: number|undefined): Observable<any> {
    ...
    return this.http.post<any>(`http://localhost:8080/api/payment/orders/paymentGateway`, combinedRequest);
  }
}